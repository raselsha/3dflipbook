!(function(modules) {
				...
				return (
					...
					(SheetBlock.prototype.getTopWorldPosition = function(v) {
						...
						for (var w = new _libs.THREE.Vector3(), _iterator3 = vs, _isArray3 = Array.isArray(_iterator3), _i5 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
							...
							var vi = _ref3;
							w.copy(vi), this.three.localToWorld(w), (v.x += 0.25 * w.x), (v.y = Math.max(w.y, w.y)), (v.z += 0.25 * w.z);
						}
						return v;
					}),
					...
					(SheetBlock.prototype.set = function(angle) {
						...
						if ("closed" === this.state) points = this.getInterpolationPoints(this.closedInterpolationIndeces, this.closedInterpolationIndeces);
							...
							if (void 0 !== closedAngle && Math.abs(closedAngle - PI / 2) > 0.01) {
								...
								points = this.getInterpolationPoints(
									this.openedInterpolationIndeces["right" === flipDirection ? (this.angle > PI / 2 ? "left" : "right") : this.angle < PI / 2 ? "left" : "right"],
						...
						if ((this.angle > PI / 2 && (this.inverse(left), this.inverse(right), (offset = -offset)), this.setPoints(left, right, offset), this.materialsCallback && this.materials))
					...
					(SheetBlock.prototype.setPoints = function(left, right, offset) {
						...
						if ((i !== this.geometry.vertices.length && console.warn("setPoints: bad mapping!"), this.corner.use && !this.corner.points.length)) {
							...
							for (var _i8 = 0, l = this.geometry.vertices.length; _i8 < l; ++_i8)
								if ((plane.projectPoint(this.geometry.vertices[_i8], proj), proj.sub(this.geometry.vertices[_i8]), proj.x * normal.x + proj.y * normal.y + proj.z * normal.z > 0)) {
					...
					(SheetBlock.prototype.interpolate = function(x, y, xi) {
						...
						for (var spline = this.aSplines[x.length], _i11 = 0; _i11 < x.length; ++_i11) spline.points[_i11].set(x[_i11], y[_i11], 0);
						return spline.getPoint(Math.min(1, Math.max(xi, 0))).y;
					}),
					(SheetBlock.prototype.loadPoints = function() {
						for (var x = [], y = [], _arr = [0, 0.2877, 0.6347, 0.8174, 1], _i12 = 0; _i12 < _arr.length; _i12++) {
							var r = _arr[_i12];
			...
			Finder = (function() {
				...
				return (
					...
					(Finder.prototype.addHits = function(p) {
						var info = this.map[
								_Utils2.default.lowerBound(this.map, {
									offset: p
								}, function(a, b) {
				...
				return (
					...
					(FullScreenX.request = function(element) {
						_FullScreen.available.call(this) ?
							...
							FullScreenX.node.hasClass(FullScreenX.classX) || ((FullScreenX.node = (0, _libs.$)(element || document.body)), FullScreenX.node.addClass(FullScreenX.classX), Promise.resolve().then(FullScreenX.handler));
					}),
					(FullScreenX.cancel = function() {
						_FullScreen.available.call(this) ?
							_FullScreen.cancel.call(this) :
							FullScreenX.node.hasClass(FullScreenX.classX) && (FullScreenX.node.removeClass(FullScreenX.classX), (FullScreenX.node = FullScreenX.defNode), Promise.resolve().then(FullScreenX.handler));
					}),
					FullScreenX
				);
	...
	function(module, exports, __webpack_require__) {
		...
		var _extends =
			...
			PATHS = window.PDFJS_LOCALE || __webpack_require__.i({
			...
			Pdf = (function() {
				function Pdf(src, loadingProgress, openOptions) {
					...
					_classCallCheck(this, Pdf),
						...
						(this.pdfJsWorker = PATHS.pdfJsLib && Pdf.isStable() ? PATHS.stablePdfJsWorker : PATHS.pdfJsWorker),
						(PDFJS ?
							Promise.resolve(PDFJS) :
							new Promise(function(resolve) {
								(PATHS.pdfJsLib ?
									new Promise(function(resolveFetch, rejectFetch) {
										_libs.$.get(Pdf.isStable() ? PATHS.stablePdfJsLib : PATHS.pdfJsLib, function(pdfJsLibCode) {
				...
				return (
					...
					(Pdf.prototype.init = function(handler, pages) {
						...
						if (pages) {
							...
							this.doubledPages ?
								(done = (2 === handler.numPages ? Promise.resolve(pages[1]) : handler.getPage(handler.numPages)).then(function(pl) {
									var pls = Pdf.getPageSize(pl);
									return (_this2.doubledLastPage = p0s.width / p0s.height / (pls.width / pls.height) < 0.75), handler;
								})) :
					...
					(Pdf.prototype.getPageType = function(n) {
						return !this.doubledPages || 0 === n || (n === this.getPagesNum() - 1 && !this.doubledLastPage) ? "full" : 1 & n ? "left" : "right";
					}),
					...
					(Pdf.prototype.getPagesNum = function() {
						return this.handler ? (this.doubledPages ? 1 + 2 * (this.handler.numPages - (1 + !this.doubledLastPage)) + !this.doubledLastPage : this.handler.numPages) : void 0;
					}),
			...
			(Array.prototype.fill = function(value) {
				...
				for (
					var O = Object(this),
						...
						final = relativeEnd < 0 ? Math.max(len + relativeEnd, 0) : Math.min(relativeEnd, len); k < final;
			...
			Book = (function(_THREE$EventDispatche) {
				...
				return (
					...
					(Book.prototype.init = function(visual, sheets, _pageCallback, props) {
						...
						(this.p = _extends({}, this.prepareProps(_extends({}, props, {
							sheets: this.isHalfBookMode() ? Math.max(props.pages - 2, 0) : sheets
						})), {
							pageCallback: function(n) {
								return _this2.isHalfBookMode() ? (n % 2 == 0 ? _extends({}, _pageCallback(n / 2), {
									number: n / 2
								}) : {
									type: "blank"
								}) : _pageCallback(n);
						...
						_CSSLayer2.default.init(visual.doc),
							...
							(this.searchEngine.onPageHitsChanged = function(page, query) {
								_this2.pageManager.refreshPageQuery(page, query), _this2.dispatchEvent({
									type: "searchResults",
						...
						if (
							(this.enableMouse(this.p.interactiveCorners),
								2 === this.getPages() && (this.enableMouse(!1), (coverP[this.p.rtl ? "left" : "right"] = _extends({}, this.p.cover, {
									width: 1e-6,
									height: 1e-6
								}))),
								(this.leftCover = new _Cover2.default(visual, _extends({}, this.p, {
									cover: coverP.left,
								...
								(this.rightCover = new _Cover2.default(visual, _extends({}, this.p, {
									cover: coverP.right,
								...
								(this.sheetBlocks = []),
								this.p.sheets > 0 && this.addSheetBlock(0, new _SheetBlock2.default(visual, _extends({}, this.p, {
									setTexture: this.setPageTexture.bind(this)
								}), 0, this.p.sheets, 0, "closed")),
							...
							(cover.materialsCallback = function(block, m, angle, state) {
								(m.opacity = _this2.p.rtl ? (2 * angle) / Math.PI : 2 - (2 * angle) / Math.PI), (m.transparent = !0);
							});
							for (var _iterator = this.sheetBlocks, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
								...
								_ref.materialsCallback = function(block, m, angle, state) {
									...
									"closed" === state
										...
										((m.opacity = 1), (m.transparent = !1), (block.three.castShadow = !0), (block.three.receiveShadow = !0)) :
										((block.three.castShadow = !1), (block.three.receiveShadow = !1), (m.opacity = _this2.p.rtl ? angle / Math.PI : 1 - angle / Math.PI), (m.transparent = !0));
								};
							}
						}
						this.set(Math.PI / 2),
							...
							setTimeout(function() {
								_this2.isProcessing() || (_this2.notifyBeforeAnimation(), _this2.notifyAfterAnimation());
							}, 100),
							this.updateThree();
					}),
					...
					(Book.prototype.calculateShadow = function() {
						var box = this.tmpBox;
						this.isHalfBookMode() ? box.setFromObject((this.p.rtl ? this.leftCover : this.rightCover).three) : box.setFromObject(this.three), this.bookShadow.scale.set(box.max.x - box.min.x, 1, box.max.z - box.min.z);
						this.bookShadow.position.set(0.5 * (box.max.x + box.min.x) - 0.015, box.min.y - 0.0015, 0.5 * (box.max.z + box.min.z) - 0.015),
					...
					(Book.prototype.getLeftFlipping = function() {
						...
						return (
							...
							(block = left) :
							(this.angle !== PI / 2 && this.angle !== (3 * PI) / 2) || (this.p.rtl && this.getPages() !== this.getBookPages()) || (block = this.leftCover),
							block
						);
					}),
					(Book.prototype.getRightFlipping = function() {
						...
						return (
							...
							(block = right) :
							(this.angle !== PI / 2 && this.angle !== (3 * PI) / 2) || ((this.p.rtl || this.getPages() === this.getBookPages()) && (block = this.rightCover)),
							block
						);
					}),
							...
							if (this.sheetPhysics.getSize() < 25) {
								...
								this.angle === PI ?
									(res = this.connectPhysics(
										...
										function(angle, height) {
											_this3.set((3 * PI) / 2 - angle / 2, 0), _this3.setSheetBlocks(angle ? PI : PI / 2 + 1e-7, "closed");
										},
										progressClb
									)) :
									left && "closed" === left.state && left.angle > PI / 2 ?
									...
									(res = this.connectPhysics(
										...
										0,
										function(angle, height) {
											_this3.set(2 * PI - angle / 2, height), angle > PI / 2 && _this3.setSheetBlocks(angle ? PI / 2 : 0, "closed");
										},
										function(angle, height) {
							...
							if (this.sheetPhysics.getSize() < 25) {
								...
								0 === this.angle ?
									(res = this.connectPhysics(
										...
										function(angle, height) {
											_this4.set(angle / 2, 0), _this4.setSheetBlocks(angle ? PI / 2 : 0, "closed");
										},
										progressClb
									)) :
									right && "closed" === right.state && right.angle <= PI / 2 ?
									...
									(res = this.connectPhysics(
										...
										0,
										function(angle, height) {
											_this4.set(PI / 2 + angle / 2, height), angle < PI / 2 && _this4.setSheetBlocks(PI / 2 + 1e-7, "closed");
										},
										function(angle, height) {
						...
						if (this.mouseController) {
							...
							if ("mouseover" === e.type) {
								if (this.hoverInfo && void 0 !== this.hoverInfo.pendings) ++this.hoverInfo.pendings;
									...
									if (n > 1 && n < 2 * (this.p.sheets + 1) && hover.physicId && (hover.angle < 0.02 || hover.angle > Math.PI - 0.02)) res = Promise.resolve(hover);
										...
										(!nextBlock || !nextBlock.physicId || (nextBlock.angle > 0.02 && nextBlock.angle < Math.PI - 0.02)) &&
										((res = this.flip(i, 1).then(function(block) {
												return block ? _this5.sheetPhysics.setParametr(block.physicId, "velocity", 0) : delete _this5.hoverInfo, block;
											})),
											(this.hoverInfo = {
												pendings: 1
											}));
									...
									res.then(function(block) {
										if ((_this5.hoverInfo && _this5.hoverInfo.pendings < 1 && ((block = void 0), delete _this5.hoverInfo), block)) {
											_this5.pageManager.turnOffEvents();
											var p = block.getProps();
											(block.force = _this5.sheetPhysics.getTargetForceClb(p.mass * block.getSize(), i ? Math.PI - 0.02 : 0.02)),
											(block.cornerForce = function() {
												return (i ? -1 : 1) * _SheetPhysics2.default.hoverCornerForceClb();
											}),
											_this5.sheetPhysics.setParametr(block.physicId, "angle", i ? Math.PI - 0.01 : 0.01),
												(_this5.hoverInfo = {
													n: n,
													block: block
												}),
					...
					(Book.prototype.removeSheetBlocks = function(first, size) {
						for (var blocks = this.sheetBlocks.splice(first, size), _iterator6 = blocks, _isArray6 = Array.isArray(_iterator6), _i6 = 0, _iterator6 = _isArray6 ? _iterator6 : _iterator6[Symbol.iterator]();;) {
							var _ref6;
							if (_isArray6) {
					...
					(Book.prototype.notifyAfterAnimation = function() {
						if (this.animationNotification) {
							var p = this.getPage();
							this.userDirection.lastTopPage !== p && ((this.userDirection.direction = Math.sign(p - this.userDirection.lastTopPage)), (this.userDirection.lastTopPage = p)),
					...
					(Book.prototype.connectPhysics = function(block, mass, angle, velocity, flexibility, coverHeight, simulateClb, removeClb, progressClb) {
						...
						return (
							(block !== this.leftCover && block !== this.rightCover) || (this.bookShadowMaterial.opacity = 0),
							res
							.then(function() {
								return (
									(block.physicId = _this8.sheetPhysics.addObject(
										...
										coverHeight,
										function(angl, ch) {
											simulateClb(angl, ch), progressClb(block, Math.abs(angle - angl) / Math.PI, "process", type()), _this8.calculateShadow(), _this8.updateThree();
										},
										function(angl, ch) {
											if ((removeClb(angl, ch), delete block.physicId, progressClb(block, Math.abs(angle - angl) / Math.PI, "finish", type()), !_this8.sheetPhysics.getSize())) {
												var p = _this8.getPage();
												(0 !== p && p !== _this8.getPages() - 1) || _this8.reducePagesWidth(!0);
											}
											Promise.resolve().then(function() {
													_this8.sheetPhysics.getSize() || (_this8.notifyAfterAnimation(), _this8.calculateShadow());
												}),
												_this8.updateThree();
										},
					...
					(Book.prototype.splitSheetBlock = function(i, leftSize) {
						...
						if (block && leftSize < block.getSize()) {
							...
							if (this.isHalfBookMode()) {
								...
								for (var _i10 = 0; _i10 < newBlock.materials.length; ++_i10)(newBlock.materials[_i10].opacity = block.materials[_i10].opacity), (newBlock.materials[_i10].transparent = block.materials[_i10].transparent);
								(newBlock.three.castShadow = block.three.castShadow), (newBlock.three.receiveShadow = block.three.receiveShadow);
							}
					...
					(Book.prototype.calcProps = function(props) {
						...
						return (
							cover.color === sheet.color || p.cover.sideTexture || (cover.sideTexture = Book.createSideTexture(cover.color, cover.side)),
						...
						return function(n, clb) {
							...
							return n >= 0 && nn < _this10.getPages() ?
								loader(nn, function(ls) {
									for (var nls = [], _iterator10 = ls, _isArray10 = Array.isArray(_iterator10), _i11 = 0, _iterator10 = _isArray10 ? _iterator10 : _iterator10[Symbol.iterator]();;) {
										var _ref10;
										if (_isArray10) {
					...
					(Book.prototype.cssLayerJsObjectInit = function(c, p) {
						...
						return {
							hide: function() {
								_this12.pendingPlayers.length && ((_this12.pendingPlayers = []), _this12.dispatchEvent({
										type: "pendingPlayers"
									})),
			...
			BookController = (function(_Controller) {
				function BookController(book, view, props) {
					...
					book.setFlipProgressClb(_this.updateViewIfState.bind(_this)),
						...
						(_this.actions.cmdSinglePage.active || (_this.actions.cmdSinglePage.activeForMobile && _this.visual.isMobile())) &&
						!_this.book.isHalfBookMode() &&
						_this.book.togleHalfBookMode(),
						(_this.state = {
							smartPan: !_this.actions.cmdSmartPan.active,
							singlePage: !(!_this.isSinglePageAvailable() || "full" !== _this.book.p.singlePageMode) && (_this.actions.cmdSinglePage.active || (_this.actions.cmdSinglePage.activeForMobile && _this.visual.isMobile())),
					...
					return (
						(_this.bookWatcher = new _Object3DWatcher2.default(_this.visual, function() {
							...
							else if (_this.book && _this.book.isHalfBookMode()) _this.p.rtl ? box0.setFromObject(book.leftCover.three) : box0.setFromObject(book.rightCover.three);
							else {
				...
				return (
					...
					(BookController.prototype.loadingAnimationHandler = function() {
						...
						visible
							...
							(delete this.pendingLoadingAnimation, this.view.setState("widLoading", {
								visible: visible
							}));
					}),
						...
						setTimeout(function() {
							if (_this4.pendingAutoPlay === pendingAutoPlay && _this4.state.autoPlay) {
								if (!_this4.book.isProcessing()) {
									var flipped = void 0;
									_this4.p.rtl ? (flipped = _this4.canFlipLeft()) && _this4.cmdBackward() : (flipped = _this4.canFlipRight()) && _this4.cmdForward(), flipped || _this4._goToPage(0);
								}
								_this4.autoPlay();
							}
					...
					(BookController.prototype.cmdSearchBtn = function() {
						this.tocCtrl && (this.tocCtrl.visible ? ("search" === this.tocCtrl.tab ? this.tocCtrl.togle() : this.tocCtrl.cmdSearch()) : (this.tocCtrl.togle(), this.tocCtrl.cmdSearch()));
					}),
					...
					(BookController.prototype.cmdSinglePage = function() {
						if (this.isSinglePageAvailable()) {
							if ("full" === this.book.p.singlePageMode)(this.state.singlePage = !this.state.singlePage), this.setBookZoom(this.bookWatcher.scale), this.updateView();
							else {
					...
					(BookController.prototype._goToPage = function(page) {
						...
						if (target != current) {
							...
							var cv = 0;
							0 === target ? ((cv = -1), (target += 1), ++covs) : target === this.book.getBookPages() - 1 && ((cv = 1), (target -= 2), ++covs),
					...
					(BookController.prototype.getPageForGUI = function() {
						...
						return this.p.rtl && (n = this.book.getBookPages() - n + 1), n > this.book.getPages() && (n = this.book.getPages()), this.book.isHalfBookMode() ? (this.p.rtl ? (n + 1) / 2 : (n - (n % 2)) / 2 + 1) : n;
					}),
						...
						return _extends({}, cmds, {
							...
							mouseCmdWheelZoom: {
								activate: function(e) {
									...
									(!_this9.embeddedInline || (_FullScreenX2.default.available() && _FullScreenX2.default.activated())) && (e.deltaY < 0 ? _this9.cmdZoomOut() : e.deltaY > 0 && _this9.cmdZoomIn()),
									...
									if (isSwipping("touchCmdSwipe"))
										if ("start" === data.state) {
											...
											((_this9.swipeData = _extends({}, _this9.swipeData, {
												...
												Math.abs(_this9.swipeData.x0 - _this9.swipeData.x) > 100 && (_this9.swipeData.x0 > _this9.swipeData.x ? _this9.cmdForward() : _this9.cmdBackward(), (_this9.swipeData.handled = !0))) :
							...
							if ((delete this.canceled, "pdf" === this.type)) {
								...
								this.useIFrame() ?
									((callManually = !!this.frame),
										this.frame || (this.frame = (0, _libs.$)('<iframe src="' + this.pdfSrc + '" style="display: none;"></iframe>').appendTo(document.body)),
		...
		var _BookPropsBuilder2 = __webpack_require__(9),
			...
			ClbBookPropsBuilder = (function(_BookPropsBuilder) {
				function ClbBookPropsBuilder(context, pageCallback, pages, onReady, style) {
					...
					if (
						(_this.calcSheets(pages), (_this.pages = pages), (_this.pageCallback = pageCallback), (_this.binds = {
											...
											_this.pageManager.getLayers(n, function(layers) {
												var finish = function() {
													if (layers.length && _this.wrappers) {
														for (
															var _block = _this.book.getBlockByPage(n), _iterator3 = layers, _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;
		...
		var _extends =
			...
			PageManager = (function() {
				function PageManager(visual, book, p) {
					...
					(this.tmpMaterialMesh = new _libs.THREE.Mesh(new _libs.THREE.PlaneGeometry(0.001, 0.001), this.tmpMaterial)),
				...
				return (
					...
					(PageManager.prototype.dispose = function() {
						...
						delete this.canvases, this.visual.removeObject(this.tmpMaterialMesh), this.visual.removeRenderCallback(this.binds.update), this.book.removeEventListener("afterAnimation", this.binds.loadPredictedPages);
					}),
					...
					(PageManager.prototype.matchImageToPage = function(image, pageN) {
						...
						return (
							...
							(bookH < image.height ? ((imageW = Math.ceil((image.width * bookH) / image.height)), (imageH = bookH)) : (imageH = image.height)), {
								x: (bookW - imageW) / 2,
								y: (bookH - imageH) / 2,
								width: imageW,
								height: imageH,
					...
					(PageManager.prototype.imageToPageTexture = function(o, n, image) {
						if (((o.match = o.match || this.matchImageToPage(image, n)), o.match.width !== o.match.pageWidth || o.match.height !== o.match.pageHeight)) {
								...
								if (o.texture) {
									...
									o.wrapper.setQuery && o.wrapper.setQuery(_this3.pageQuery),
										...
										(o.wrapper.onChange = function(image, canceled) {
											canceled
												?
												("queuedForRender" !== o.state && ((o.state = "loaded"), o.wrapper && _this3.pushInRenderQueue(_this3.rendering)), delete _this3.rendering) :
												o.texture &&
					...
					(PageManager.prototype.calcResolution = function(o) {
						...
						if (this.p.autoResolution.enabled) {
							var k =
								...
								Math.min(((this.isSinglePage(o) || this.book.isHalfBookMode() ? 1 : 0.5) * this.visual.width()) / o.widthTexels, this.visual.height() / o.heightTexels);
							res = {
						...
						if (this.canvases) {
							...
							if (this.rendering) this.rendering.wrapper && this.rendering.wrapper.setRenderPause(!p.renderWhileFlipping && (this.book.isProcessing() || this.visual.getOrbit().isMoving())), this.book.updateThree();
								...
								if (((this.rendering = top || active), ((this.isMobile() && p.renderInactivePagesOnMobile) || (!this.isMobile() && p.renderInactivePages)) && !this.rendering))
									for (
										var ud = this.book.getUserDirection(), near = {}, _iterator8 = this.renderQueue, _isArray8 = Array.isArray(_iterator8), _i8 = 0, _iterator8 = _isArray8 ? _iterator8 : _iterator8[Symbol.iterator]();;
						...
						if (this.transferEvents) {
							...
							if (toObject.wrapper) {
								...
								this.pageCache.forEach(function(ent) {
									var o = ent[1];
									if (o.simulate)
										if (toObject !== o || !o.match || (o.match.width === o.match.pageWidth && o.match.height === o.match.pageHeight)) o.simulate(e, toDoc, uv.x, uv.y);
										else {
											var x = uv.x * o.match.pageWidth,
												y = (1 - uv.y) * o.match.pageHeight;
											x > o.match.x && x < o.match.x + o.match.width && y > o.match.y && y < o.match.y + o.match.height ?
			...
			PdfLinksHandler = (function() {
				...
				return (
					...
					(PdfLinksHandler.prototype.defaultHandler = function(type, destination) {
						"internal" === type ? this.ctrl.goToPage(destination) : "external" === type && -1 === destination.toLowerCase().search("javascript:") && window.open(destination, "_blank");
					}),
				...
				return (
					...
					(SearchEngine.prototype.process = function() {
						...
						if (this.update) {
							...
							if (((this.update = !1), (this.page = 0), (this.stamp = Date.now()), this.query.length > 1)) this.process();
							...
							this.find(this.pageCallback(this.page)).then(function(contexts) {
								stamp === _this.stamp &&
									(contexts.length && _this.results.push({
										page: _this.page,
										contexts: contexts
				...
				return (
					...
					(SheetPhysics.prototype.findCollisions = function(os) {
						for (var ci = {
							...
							(os[i - 1].l > os[i].l && ++ci.num, ++ci.num, (ci.last = i), -1 === ci.all.indexOf(i - 1) && ci.all.push(i - 1), -1 === ci.all.indexOf(i) && ci.all.push(i));
						return ci;
					}),
		...
		var SoundsEnviroment = (function() {
			function SoundsEnviroment() {
				...
				_classCallCheck(this, SoundsEnviroment),
					...
					(this.audio = {}),
					this.sounds.startFlip && ((this.audio.startFlip = new Audio(this.sounds.startFlip)), (this.audio.startFlip.volume = 0.5)),
					this.sounds.endFlip && ((this.audio.endFlip = new Audio(this.sounds.endFlip)), (this.audio.endFlip.volume = 0.5));
			}
			return (
						...
						if (_Detector2.default.webgl) {
							...
							options.propertiesCallback &&
								...
								(scene.bookBuilder = function(props, sheets, pageCallback) {
									...
									(scene.thumbnails = new _Thumbnails2.default(scene.visual, scene.view.getThumbnailsView(), pageCallback, scene.book.getRealPages(), {
									...
									scene.tocCtrl.setThumbnails(scene.thumbnails),
										options.outline && ((scene.bookmarks = new _Bookmarks2.default(scene.view.getBookmarksView(), options.outline)), scene.tocCtrl.setBookmarks(scene.bookmarks)),
										...
										((scene.search = new _Search2.default(scene.view.getSearchView(), scene.book.getRealPages())),
											...
											test.src.getHandler(function(handler) {
												handler.getOutline().then(function(outline) {
													outline &&
														outline.length &&
														((scene.bookmarks = new _Bookmarks2.default(scene.view.getBookmarksView(), outline)), scene.tocCtrl.setBookmarks(scene.bookmarks, test.src), scene.ctrl.setTocCtrl(scene.tocCtrl));
												});
											}),
									...
									(scene.shareCtrl = new _ShareController2.default(
										...
										options.shareLinkBuilder :
										function(page) {
											return new RegExp("([?&])" + autoNavigator.urlParam + "=[0-9]+").test(location.href) ?
												location.href.replace(new RegExp("([?&])" + autoNavigator.urlParam + "=[0-9]+"), "$1" + autoNavigator.urlParam + "=" + page) :
												location.href.split("#")[0] + (~location.href.indexOf("?") ? "&" : "?") + autoNavigator.urlParam + "=" + page + location.hash;
										}
									)),
							...
							options.pdf ?
								...
								(options.onPageCallbackError && options.onPageCallbackError.push(options.error || onError),
									...
									if (cmd.hasClass("cmd")) {
										...
										cmd.hasClass("togle") ?
											((node.minimized = !node.minimized),
												...
												(li.find(".togle").removeClass("minimized"), li.append((_this.renderNode(node).match(/<ul(.|\n)*<\/ul>/g) || [""])[0]))) :
				...
				function DocMouseSimulator(jFrame, element) {
					...
					for (
						var terms = [{
							...
							i = 0; i < this.doc.styleSheets.length;
						++i
					)
					...
					(LoadingAnimation.prototype.update = function(T) {
						if (((this.os = this.integrate(T)), (this.t += T), this.t >= this.p.updateInterval)) {
							...
							for (var i = 0, _a = a, _r = 0.2 * r; i < 10; ++i, _r *= 0.9, _a += da) {
								ctx.beginPath(), (ctx.fillStyle = _GraphUtils2.default.color2Rgba(_GraphUtils2.default.inverseColor(this.p.color, (10 - i) / 10), (0.7 * (10 - i)) / 10));
					...
					(_this.update = (function() {
						...
						return function() {
							...
							return (
								...
								!0 === scope.enableDamping ? ((sphericalDelta.theta *= 1 - scope.dampingFactor), (sphericalDelta.phi *= 1 - scope.dampingFactor)) : sphericalDelta.set(0, 0, 0),
								(scale = 1),
								panOffset.set(0, 0, 0),
				...
				return (
					...
					(Search.prototype.doQuery = function() {
						...
						if (this.onQuery) {
							...
							setTimeout(function() {
								queryStamp === _this.queryStamp && ((_this.lastQuery = _this.query[0].value.trim()), _this.onQuery(_this.lastQuery));
							}, 1e3);
						}
					}),
					(Search.prototype.setResults = function(results, lastPage) {
						...
						(0, _libs.$)(htmls.join("")).appendTo(this.results),
							...
							void 0 === lastPage ? this.status.html("") : this.status.html(lastPage + 1 + " " + (0, _libs.tr)("of") + " " + this.pages);
					}),
					Search
				);
			})();
			...
			Thumbnails = (function() {
				...
				return (
					...
					(Thumbnails.prototype.setImage = function(thumbnail, img) {
						...
						(thumbnail.img = img),
						thumbnail.thumbnail && (thumbnail.thumbnail.css("background-image", ["url('", img, "')"].join("")), thumbnail.thumbnail.removeClass("loading")),
					...
					(Thumbnails.prototype.render = function() {
						for (var elements = ['<div class="thumbnails">'], i = 0; i < this.size; ++i)
							elements.push(
								['<div class="item"><a><div class="thumbnail loading" style="padding-top:' + Math.round(100 / this.p.kWtoH) + '%;"></div></a><div class="heading"><a title="', i + 1, '">', i + 1, "</a></div></div>"].join("")
							);
			...
			View = (function() {
				function View(parentContainer, onLoad) {
					...
					var build = function build() {
						_this3.isIOS && ((0, _libs.$)(_this3.parentContainer[0].ownerDocument.defaultView).on("resize", _this3.binds.onResize), _this3.onResize(), setTimeout(_this3.checkIframeSize.bind(_this3), 250)),
